...
public abstract class GLTexture implements Disposable {
	...
	public GLTexture (int glTarget) {
		this(glTarget, GLContext.getGL20().glGenTexture ());
	}
	...
	public void bind () {
		GLContext.getGL20().glBindTexture(glTarget, glHandle);
	}
	...
	public void bind (int unit) {
		GLContext.getGL20().glActiveTexture(GL20.GL_TEXTURE0 + unit);
		GLContext.getGL20().glBindTexture(glTarget, glHandle);
	}
	...
	public void unsafeSetWrap (TextureWrap u, TextureWrap v, boolean force) {
		if (u != null && (force || uWrap != u)) {
			GLContext.getGL20().glTexParameteri(glTarget, GL20.GL_TEXTURE_WRAP_S, u.getGLEnum());
			uWrap = u;
		}
		if (v != null && (force || vWrap != v)) {
			GLContext.getGL20().glTexParameteri(glTarget, GL20.GL_TEXTURE_WRAP_T, v.getGLEnum());
			vWrap = v;
		}
	}
	...
	public void unsafeSetFilter (TextureFilter minFilter, TextureFilter magFilter, boolean force) {
		if (minFilter != null && (force || this.minFilter != minFilter)) {
			GLContext.getGL20().glTexParameteri(glTarget, GL20.GL_TEXTURE_MIN_FILTER, minFilter.getGLEnum());
			this.minFilter = minFilter;
		}
		if (magFilter != null && (force || this.magFilter != magFilter)) {
			GLContext.getGL20().glTexParameteri(glTarget, GL20.GL_TEXTURE_MAG_FILTER, magFilter.getGLEnum());
	...
	protected void delete () {
		if (glHandle != 0) {
			GLContext.getGL20().glDeleteTexture (glHandle);
			glHandle = 0;
		}
	}
	...
	public static void uploadImageData (int target, TextureData data, int miplevel) {
		...
		if (data.useMipMaps()) {
			...
			GLContext.getGL20().glTexImage2D(target, miplevel, pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(), 0,
