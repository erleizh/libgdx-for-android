...
// }
	...
	private int linkProgram (int program) {
		...
		if (linked == 0) {
// GLContext.getGL20().glGetProgramiv(program, GL20.GL_INFO_LOG_LENGTH, intbuf);
// int infoLogLength = intbuf.get(0);
// if (infoLogLength > 1) {
			log = GLContext.getGL20().glGetProgramInfoLog(program);
// }
			return -1;
		}
...
// }
	...
	private void fetchUniforms () {
		params.clear();
		GLContext.getGL20().glGetProgramiv(program, GL20.GL_ACTIVE_UNIFORMS, params);
		...
		for (int i = 0; i < numUniforms; i++) {
			...
			params.put(0, 1);
			type.clear();
			String name = GLContext.getGL20().glGetActiveUniform(program, i, params, type);
			int location = GLContext.getGL20().glGetUniformLocation(program, name);
			uniforms.put(name, location);
	...
	private void fetchAttributes () {
		params.clear();
		GLContext.getGL20().glGetProgramiv(program, GL20.GL_ACTIVE_ATTRIBUTES, params);
		...
		for (int i = 0; i < numAttributes; i++) {
			...
			params.put(0, 1);
			type.clear();
			String name = GLContext.getGL20().glGetActiveAttrib(program, i, params, type);
			int location = GLContext.getGL20().glGetAttribLocation(program, name);
			attributes.put(name, location);
