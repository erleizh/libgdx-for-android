...
public class Timer {
    ...
    static private TimerThread thread() {
        synchronized (threadLock) {
            if (thread == null || thread.files != GLContext.getFiles()) {
    ...
    synchronized long update(long timeMillis, long waitMillis) {
        for (int i = 0, n = tasks.size; i < n; i++) {
            ...
            synchronized (task) {
                ...
                task.mGLContext.postRunnable(task);
            }
        }
        return waitMillis;
    }
    ...
    static class TimerThread implements Runnable, LifecycleListener {
        ...
        public void run() {
            while (true) {
                synchronized (threadLock) {
                    if (thread != this || files != GLContext.getFiles()) break;
